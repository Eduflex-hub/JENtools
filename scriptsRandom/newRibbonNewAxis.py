...
def ribbon (_objA, _objB, _divisions, _name):
	...
	if mc.objExists(_objA.replace('JNT', 'twistLOC'))==False:
	...
	if mc.objExists(_objB.replace('JNT', 'twistLOC'))==False:
		twistLocB = mc.spaceLocator(n= _objB.replace('JNT', 'twistLOC'))
		twistLOCGRPB=mc.createNode('transform', n=_objB.replace('JNT', 'twistLOCGRP') )
		mc.parent(twistLocB, twistLOCGRPB)	
		mc.delete(mc.parentConstraint(_objB, twistLOCGRPB))
	
	

ribbon(sel[0], sel[1], 3, sel[0].replace('JNT', 'NURBS'))
