...
def dynChain():
    ...
    for i in allJ:
        ...
        mc.select(cl=1)
        dynJoint = mc.joint(n=i.replace('JNT', 'dynJoint'), rad=0.1)
        mc.select(cl=1)
        driverJoint = mc.joint(n=i.replace('JNT', 'driverJoint'), rad=0.1)
        ...
        if child != None:
            ztr = mc.createNode('transform', n=i.replace('JNT', 'dynZTR'))
            dynGrp = mc.createNode('transform', n=i.replace('JNT', 'dynControlGRP'))
            CTRL = mc.circle(n=i.replace('JNT', 'dynCTRL'), nr=(1, 0, 0), r=0.5)
        ...
        if parent != None:
            dynParent = parent[0].replace('JNT', 'dynJoint')
            driverParent = parent[0].replace('JNT', 'driverJoint')
            controlParent = parent[0].replace('JNT', 'dynCTRL')
    ...
    curve = mc.rename(curve, first.replace('dynJoint', 'setCurve'))

    curveAux = mc.curve(p=curveCoords, d=1)
    curveAux = mc.rename(curveAux, first.replace('dynJoint', 'auxSetCurve'))

    skinCurve = mc.curve(p=curveCoords, d=1)
    skinCurve = mc.rename(skinCurve, first.replace('dynJoint', 'skinCurve'))
    ...
    dynCurve = mc.rename(dynCurve, first.replace('dynJoint', 'dynCurve'))
    ...
    follicleGRP = mc.rename(follicleGRP, first.replace('dynJoint', 'follGRP'))

    follicle = mc.ls('follicle1')[0]
    follicle = mc.rename(follicle, first.replace('dynJoint', 'follicle'))
    ...
    hairSystem = mc.rename(hairSystem, first.replace('dynJoint', 'hairSystem'))

    condMethod = mc.createNode('condition', n=first.replace('dynJoint', 'timeToMethodCondition'))
    ...
    mc.setAttr(condMethod + '.colorIfFalseR', 3)
    mc.connectAttr(condMethod + '.outColorR', hairSystem + '.simulationMethod')

    splineIK = mc.ikHandle(sj=first, ee=last, sol='ikSplineSolver', c=dynCurve,
                           scv=0, fj=1, n=first.replace('dynJoint', 'dynSplineHandle'))[0]

    # mc.select(allDrivers)
    # mc.select(skinCurve, add=1)
    # mc.skinCluster(n=dynCurve.replace('dynCurve', 'dynSkinCluster'), tsb=1)
    ...
    for x in clus:
        index = clus.index(x)
        x = mc.rename(x, first.replace('dynJoint', '' + str(index) + 'clusCurve'))
        control = x.replace('01_' + str(index) + 'clusCurve', '0' + str(index + 1) + 'dynCTRL')

        print control
        if mc.objExists(control) is True:
            mc.parentConstraint(control, x, mo=1)
        else:
            control = x.replace('01_' + str(index) + 'clusCurve', '0' + str(index) + 'dynCTRL')
            mc.parentConstraint(control, x, mo=1)
        mc.setAttr(x + 'Cluster.relative', 0)

    for x in allJ:
        y = x.replace('JNT', 'dynJoint')
        z = x.replace('JNT', 'driverJoint')
        cons = mc.parentConstraint(z, y, x, mo=0)
        mc.setAttr(cons[0] + '.interpType', 2)
        rev = mc.createNode('reverse', n=x.replace('JNT', 'dynReverse'))
    ...
    mc.setAttr(splineIK + '.dWorldUpVectorZ', 1)
    mc.connectAttr('C_hip_01_CTRL.worldMatrix[0]', splineIK + '.dWorldUpMatrix')
