...
def dynChain():
    ...
    splineIK = mc.ikHandle(sj=first, ee=last, sol='ikSplineSolver', c=dynCurve,
                           scv=0, fj=1, n=first.replace('_dynJoint', '_dynSplineHandle'))[0]
    ...
    for x in allJ:
        ...
        cons = mc.parentConstraint(z, y, x, mo=1)
    ...
    mc.setAttr(splineIK + '.dWorldUpVectorZ', 1)
    mc.connectAttr('C_head_01_CTRL.worldMatrix[0]', splineIK + '.dWorldUpMatrix')
